...
def calculate_adx(highs, lows, closes, period=14):
    ...
    for i in range(period, len(tr_list)):
        atr = (tr_list[i] + atr * (period - 1)) / period
        smoothed_plus_dm = (plus_dm_list[i] + smoothed_plus_dm * (period - 1)) / period
        smoothed_minus_dm = (minus_dm_list[i] + smoothed_minus_dm * (period - 1)) / period
    if atr == 0: return 25
...
def get_multi_timeframe_klines():
    # رفع الـ limit إلى 100 شمعة لحسابات فنية أكثر دقة واستقراراً لحساب الإشارات
    timeframes = {
    ...
    for tf_name, params in timeframes.items():
        try:
            url = f"https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval={params['interval']}&limit={params['limit']}"
...
# --- الـ Worker الخلفي المصلح بالكامل ---
def background_radar_worker():
    global stats
    print("Background Radar Started Successfully.")
    while True:
        try:
            try:
                time_url = "https://api.binance.com/api/v3/time"
                server_time_ms = requests.get(time_url, timeout=3).json()['serverTime']
                now = datetime.fromtimestamp(server_time_ms / 1000, tz=timezone.utc)
                binance_time = now.strftime("%H:%M:%S")
            except:
                binance_time = datetime.now(timezone.utc).strftime("%H:%M:%S")
            ...
            with state_lock:
                ...
                # 1. إدارة الصفقات الحية المفتوحة
                ...
                # 2. منطق تحليل وتوليد الإشارات المعدل بالكامل لمنع التجمد
                else:
                    ...
                    if multi_tf and '15m' in multi_tf:
                        ...
                        for tf_name, data in multi_tf.items():
                            ...
                            # حساب النقاط بناءً على المؤشرات الاتجاهية والزخم
                            score = 0
                        ...
                        # حساب المتوسط المرجح للنقاط
                        final_score = (signals.get('15m', 0) * 3 + signals.get('1h', 0) * 2 + signals.get('4h', 0) * 1) / 6
                        
                        # طباعة تتبع حي في السيرفر لمساعدتك في معرفة حركة النقاط الحالية
                        print(f"[{binance_time}] Price: {current_price} | 15m Score: {signals.get('15m', 0)} | Final Score: {final_score:.2f}")

                        stats["tp1_hit"] = False
                        stats["tp2_hit"] = False
                        stats["tp3_hit"] = False
                        stats["trailing_activated"] = False
                        stats["trailing_stop"] = 0
                        
                        state_triggered = False
                        
                        # عتبة الحساسية مضبوطة الآن بدقة (1.0 لسرعة الاستجابة، ويمكن رفعها لـ 1.5 للأمان)
                        if final_score >= 1.0 and atr_15m > 0:
                            stats["active_signal"] = "BUY"
                            stats["entry_price"] = current_price
                            stats["highest_price"] = current_price
                            stats["lowest_price"] = current_price
                            stats["tp1"] = current_price + (atr_15m * 1.5)
                            stats["tp2"] = current_price + (atr_15m * 3.0)
                            stats["tp3"] = current_price + (atr_15m * 5.0)
                            stats["sl"] = current_price - (atr_15m * 2.0)
                            stats["confidence"] = f"{min(95, int(abs(final_score) * 25))}%"
                            state_triggered = True
                            print(f"📢 SIGNAL TRIGGERED: BUY at {current_price}")
                            ...
                            state_triggered = True
                            print(f"📢 SIGNAL TRIGGERED: SELL at {current_price}")
                        else:
                            stats["active_signal"] = "WAITING"
                            stats["confidence"] = "0%"
        ...
        <script>
            const translations = {
                en: { 
                    ...
                    adv_wait: "System is scanning multiple timeframes (15m/1h/4h). Once a trade is active, step-by-step risk management instructions will appear here.",
                ...
                ar: { 
                    ...
                    adv_wait: "النظام يحلل أطر زمنية متعددة (15 دقيقة/ساعة/4 ساعات). بمجرد دخول الصفقة ستظهر خطة تحريك الوقف خطوة بخطوة.",
            ...
            function changeLanguage(lang) {
                ...
                document.getElementById('lbl-time').innerText = translations[lang].lbl_time; 
                document.getElementById('lbl-wins').innerText = translations[lang].wins;
                document.getElementById('lbl-losses').innerText = translations[lang].losses; 
                document.getElementById('lbl-entry').innerText = translations[lang].entry;
                document.getElementById('lbl-tp1').innerText = translations[lang].tp1; 
                document.getElementById('lbl-tp2').innerText = translations[lang].tp2;
                document.getElementById('lbl-tp3').innerText = translations[lang].tp3; 
                document.getElementById('lbl-sl').innerText = translations[lang].sl;
                document.getElementById('lbl-conf').innerText = translations[lang].conf; 
            ...
            function renderData() {
                ...
                document.getElementById('count-wins').innerText = globalData.stats.wins; 
                document.getElementById('count-losses').innerText = globalData.stats.losses;
                document.getElementById('binance-clock').innerText = btc.binance_time; 
                document.getElementById('btc-price').innerText = "$" + btc.price;
                document.getElementById('btc-entry').innerText = btc.entry === "—" ? "—" : "$" + btc.entry;
                document.getElementById('btc-tp1').innerText = btc.tp1 === "—" ? "—" : "$" + btc.tp1; 
                document.getElementById('btc-tp2').innerText = btc.tp2 === "—" ? "—" : "$" + btc.tp2;
                document.getElementById('btc-tp3').innerText = btc.tp3 === "—" ? "—" : "$" + btc.tp3; 
                document.getElementById('btc-sl').innerText = btc.sl === "—" ? "—" : "$" + btc.sl;
